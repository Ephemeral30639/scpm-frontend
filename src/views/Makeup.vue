...
export default defineComponent({
  data() {
    ...
    const loadAll = () => {
      ...
      .then(response => {
          ...
          axios.get('https://localhost:5000/makeup/getactivecourses', {params:{trimester: trimester}})
          .then((response) => {
            if(response.data == 'Error'){
  ...
  methods: {
    handleSelect (item) {
      ...
      axios.get('https://localhost:5000/makeup/getmakeup', {params:{course: item.value, trimester: this.trimester}})
      .then(response => {
        if(response.data == 'Error'){
